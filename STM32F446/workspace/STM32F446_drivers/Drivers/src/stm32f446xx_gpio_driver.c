...
void GPIO_Init(GPIO_Handle_t *pGPIOHandle){
	...
	if(pGPIOHandle->GPIO_Pin_Config.GPIO_PinMode<=GPIO_MODE_ANALOG){
		...
		pGPIOHandle->pGPIOx->MODER &= ~(0x3<<pGPIOHandle->GPIO_Pin_Config.GPIO_PinNumber);//Clearing
	...
	pGPIOHandle->pGPIOx->OSPEEDR &= ~(0x3<<pGPIOHandle->GPIO_Pin_Config.GPIO_PinNumber);//Clearing
	pGPIOHandle->pGPIOx->OSPEEDR |= temp;

	temp=0;
	//3. configure the pupd setting(pull-up pull-down)
	...
	temp=(pGPIOHandle->GPIO_Pin_Config.GPIO_PinOPType << (2 * pGPIOHandle->GPIO_Pin_Config.GPIO_PinNumber));
...
uint16_t GPIO_ReadFromInputPort(GPIO_RegDef_t *pGPIOx){
	//port is of 16 pins thats why uint_16
	uint16_t value;
...
void GPIO_ToggleOutputPin(GPIO_RegDef_t *pGPIOx, uint8_t PinNumber){
	 ...
	 pGPIOx->ODR ^= (1<<PinNumber);  //or pGPIOx ->ODR ^ (1<<PinNumber)
