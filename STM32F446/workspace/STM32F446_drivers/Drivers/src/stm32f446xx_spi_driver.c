...
void SPI_Init(SPI_Handle_t *pSPIHandle){
	...
	tempreg |= pSPIHandle->SPIConfig.SPI_CPOL << SPI_CR1_BR;
...
static void spi_txe_interrupt_handle(SPI_Handle_t *pSPIHandle){
	...
	//2. check the DFF bit in CR1
			...
			if(!pSPIHandle->TxLen){
				...
				//close the SPI transmission if TxLen is zero, and inform the application that TX is over.
...
static void spi_ovr_err_interrupt_handle(SPI_Handle_t *pSPIHandle){
	...
	//clear the OVR(OVERRUN) flag and then inform the application
